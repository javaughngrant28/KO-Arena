...
function FuncLib.SetCollisionGroup(object: Instance, collisionGroupName: string)
...
function FuncLib.GetDistanceBetweenParts(part1: BasePart, part2: BasePart): number
...
function FuncLib.AdjustPropertiesByType(target: Instance, instanceType: string, properties: { [string]: any }): nil
...
function FuncLib.Color3ToRGBFormat(color3: Color3)
...
function FuncLib.SetAttributes(instance: Instance, list: { [string]: any })
...
function FuncLib.TweenCreate(timer: number, instance: Instance, value: {}, EasingStyle: Enum.EasingStyle, EasingDirection: Enum.EasingDirection, doesDestroy: boolean, callback: any): Tween
...
function FuncLib.LoadAnimationTracks(animations: {Animation}, humanoid: Humanoid): {[string]: AnimationTrack}
...
function FuncLib.PlayAnimationForDuration(animationTrack: AnimationTrack, duration: number)
...
function FuncLib.DisconnectAllConnections(connectionsTable: table)
    for key, value in pairs(connectionsTable) do
        if typeof(value) == "RBXScriptConnection" then
            value:Disconnect()
            connectionsTable[key] = nil
        elseif typeof(value) == "table" then
            FuncLib.DisconnectAllConnections(value)
        end
    end
end
...
function FuncLib.CloneSounds(sounds: {}, targetFolder: Folder)
